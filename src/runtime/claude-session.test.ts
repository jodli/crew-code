...
describe("runtime/claude-session", () => {
  describe("claudeSessionPath()", () => {
    test("encodes cwd by replacing slashes and dots with dashes", () => {
      ...
      expect(path).toBe(
        join(homedir(), ".claude", "projects", "-home-user-repos-my-project", "abc-123.jsonl"),
      );
    });
    ...
    test("handles root cwd", () => {
      ...
      expect(path).toBe(
        join(homedir(), ".claude", "projects", "-", "sess-1.jsonl"),
      );
    });
    ...
    test("handles deeply nested path", () => {
      ...
      expect(path).toBe(
        join(homedir(), ".claude", "projects", "-a-b-c-d", "id.jsonl"),
      );
    });
  });
