...
import type { TeamSummary, MemberDetail, TeamDetail } from "@crew/core/status.ts";
...
export const FIXTURE_BLUEPRINTS: Blueprint[] = [
  {
    ...
    agents: [
      ...
      { name: "security-reviewer", agentType: "general-purpose", model: "sonnet", color: "#f7768e", prompt: "You are a security-focused code reviewer." },
      { name: "test-writer", agentType: "general-purpose", model: "haiku", color: "#9ece6a", prompt: "You write tests for the reviewed code." },
    ],
  },
  {
    ...
    agents: [
      ...
      { name: "backend-dev", agentType: "general-purpose", model: "sonnet", color: "#7aa2f7", cwd: "~/repos/api", prompt: "You implement backend features." },
    ],
  },
  {
    name: "docs-generator",
    ...
    agents: [
      ...
      { name: "api-docs", agentType: "general-purpose", model: "haiku", color: "#7dcfff", prompt: "Generate API documentation." },
    ],
  },
];

export const FIXTURE_AGENT_TYPES: string[] = [
  "general-purpose", "team-lead", "code-simplifier",
  "test-runner", "codebase-analyzer", "web-search-researcher",
];
...
const FIXTURE_MEMBERS_REVIEW: MemberDetail[] = [
  { name: "team-lead", agentId: "team-lead@code-review-team", agentType: "team-lead", model: "opus", cwd: "~/repos/project", processId: 48201, unreadCount: 0 },
  { name: "security-reviewer", agentId: "security-reviewer@code-review-team", agentType: "general-purpose", model: "sonnet", color: "#f7768e", cwd: "~/repos/project", processId: 48202, unreadCount: 1 },
  { name: "test-writer", agentId: "test-writer@code-review-team", agentType: "general-purpose", model: "haiku", color: "#9ece6a", cwd: "~/repos/project", processId: 48203, unreadCount: 0 },
];

const FIXTURE_MEMBERS_FEATURE: MemberDetail[] = [
  { name: "team-lead", agentId: "team-lead@my-feature", agentType: "team-lead", model: "opus", cwd: "~/repos/app", processId: 49001, unreadCount: 0 },
  { name: "backend-dev", agentId: "backend-dev@my-feature", agentType: "general-purpose", model: "sonnet", color: "#7aa2f7", cwd: "~/repos/api", unreadCount: 2 },
  { name: "frontend-dev", agentId: "frontend-dev@my-feature", agentType: "general-purpose", model: "sonnet", color: "#bb9af7", cwd: "~/repos/web", unreadCount: 0 },
];

export const FIXTURE_TEAM_DETAILS: Record<string, TeamDetail> = {
  "code-review-team": { name: "code-review-team", description: "Automated code review squad", members: FIXTURE_MEMBERS_REVIEW },
...
const FIXTURE_MESSAGES: InboxMessage[] = [
  ...
  { from: "security-reviewer", text: "Found 2 potential XSS vulnerabilities.", timestamp: min(10), color: "#f7768e", read: true },
...
export const handlers = [
  // Health
  http.get("/api/health", () =>
    HttpResponse.json({ status: "ok", version: "0.1.6", uptime: 12345 }),
  ),

  // Blueprints
  http.get("/api/blueprints", () =>
    HttpResponse.json(FIXTURE_BLUEPRINTS),
  ),

  http.get("/api/blueprints/:name", ({ params }) => {
    const bp = FIXTURE_BLUEPRINTS.find((b) => b.name === params.name);
    if (!bp) return HttpResponse.json({ error: { kind: "blueprint_not_found", message: `Blueprint '${params.name}' not found` } }, { status: 404 });
  ...
  http.post("/api/blueprints/:name/load", ({ params }) => {
    ...
    const detail: TeamDetail = {
      ...
      members: bp.agents.map((a) => ({
        name: a.name, agentId: `${a.name}@${bp.name}`, agentType: a.agentType ?? "general-purpose",
        model: a.model, color: a.color, cwd: a.cwd ?? ".", unreadCount: 0,
      })),
    };
  ...
  http.get("/api/agent-types", () =>
    HttpResponse.json(FIXTURE_AGENT_TYPES),
  ),

  http.get("/api/models", () =>
    HttpResponse.json(FIXTURE_MODELS),
  ),

  // Teams
  http.get("/api/teams", () =>
    HttpResponse.json(FIXTURE_TEAM_SUMMARIES),
  ),

  http.get("/api/teams/:name", ({ params }) => {
    const detail = FIXTURE_TEAM_DETAILS[params.name as string];
    if (!detail) return HttpResponse.json({ error: { kind: "team_not_found", message: `Team '${params.name}' not found` } }, { status: 404 });
  ...
  http.post("/api/teams/:name/agents/:agent/stop", () =>
    HttpResponse.json({ stopped: true }),
  ),

  http.delete("/api/teams/:name/agents/:agent", () =>
    new HttpResponse(null, { status: 204 }),
  ),

  // Messages
  http.get("/api/teams/:name/agents/:agent/inbox", () =>
    HttpResponse.json(FIXTURE_AGENT_INBOX),
  ),

  http.post("/api/teams/:name/agents/:agent/inbox", () =>
    new HttpResponse(null, { status: 201 }),
  ),

  http.get("/api/teams/:name/messages", () =>
    HttpResponse.json(FIXTURE_CREW_MESSAGES),
  ),
];
