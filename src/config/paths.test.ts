...
import { claudeInboxesDir, claudeInboxPath, claudeTeamConfigPath, claudeTeamDir, claudeTeamsDir } from "./paths.ts";
...
describe("config/paths", () => {
  ...
  test("claudeTeamDir(name) returns ~/.claude/teams/{name}", () => {
    expect(claudeTeamDir("my-team")).toBe(join(home, ".claude", "teams", "my-team"));
  });

  test("claudeTeamConfigPath(name) returns ~/.claude/teams/{name}/config.json", () => {
    expect(claudeTeamConfigPath("my-team")).toBe(join(home, ".claude", "teams", "my-team", "config.json"));
  });

  test("claudeInboxesDir(name) returns ~/.claude/teams/{name}/inboxes", () => {
    expect(claudeInboxesDir("my-team")).toBe(join(home, ".claude", "teams", "my-team", "inboxes"));
  });
